...
fn resolve_node_path(active_path: &str, node_path: &str) -> PathBuf {
...
fn open_containing_folder(path: &Path) -> std::io::Result<()> {
...
pub fn ModuleTree() -> Element {
    ...
    let is_node_file_manager = graph_view_mode.read().as_str() == "node_file_manager";

    rsx! {
        if is_node_file_manager {
            NodeFileManager {}
        } else {
            ArchitectureModuleTree {}
        }
    }
}

fn ArchitectureModuleTree() -> Element {
